...
def plot_data(X, y, title="Dataset"):
    ...
    plt.scatter(
        X[y.flatten() == 0, 0], X[y.flatten() == 0, 1], label="Class 0", alpha=0.7
    )
    plt.scatter(
        X[y.flatten() == 1, 0], X[y.flatten() == 1, 1], label="Class 1", alpha=0.7
    )
    ...
    if not os.environ.get("HIDE_PLOTS", False):
        plt.show()
...
def plot_decision_boundary(model, X, y, title="Decision Boundary", save_fname=None):
    ...
    # Plot the contour
    plt.contourf(
        xx1, xx2, Z, alpha=0.3, levels=[0, 0.5, 1], colors=["#3B82F6", "#F472B6"]
    )
    ...
    if save_fname:
        if not os.environ.get("HIDE_PLOTS", False):
            plt.savefig(save_fname)  # Save the plot
...
class LogisticRegression:
    ...
    def _sigmoid(self, z):
        # PERF: (1a) Implement sigmoid function
    ...
    def transform(self, X):
        ...
        # PERF: (1b) implement feature transform
    ...
    def _compute_cost(self, y, y_pred):
        m = len(y)
        # PERF: (1c) implement cost function
        ...
        # prevent log 0 and upper bound excess
        y_pred_clipped = np.clip(y_pred, EPSILON, 1 - EPSILON)
        cost = (
            -np.sum(y * np.log(y_pred_clipped) + (1 - y) * np.log(1 - y_pred_clipped))
            / m
        )
        ...
        if self.regularization == "L2":
            # PERF: (2a) implement L2 Regularization
            reg_cost = self.lambda_val / (2 * m) * np.linalg.norm(self.weights) ** 2

        elif self.regularization == "L1":
            # PERF: (2b) implement L1 Regularization
    ...
    def fit(self, X, y):
        ...
        for i in range(self.n_iterations):
            ...
            # PERF: (1a) update weights and bias
...
plot_decision_boundary(
    ...
    "submission_4a_linear_decision_boundary.png",
)

nonlinear_model = LogisticRegression(
    learning_rate=0.1, n_iterations=20000, poly_features=True
)
...
plot_decision_boundary(
    ...
    X,
    y,
    "Non-linear Decision Boundary (Succeeds)",
    "submission_4a_nonlinear_decision_boundary.png",
)
...
if not os.environ.get("HIDE_PLOTS", False):
    plt.savefig("submission_4b.png")  # Save the plot
...
plot_decision_boundary(
    ...
    X,
    y,
    f"Best Model (lr={best_params['learning_rate']}, lambda={best_params['lambda_val']})",
    "submission_4c.png",
)
    ...
    for epoch in range(n_epochs):
        ...
        for i in range(0, n_samples, batch_size if optimizer != "stochastic" else 1):
            ...
            cost = (
                -1
                / m_batch
                * np.sum(y_batch * np.log(y_pred) + (1 - y_batch) * np.log(1 - y_pred))
            )
            epoch_cost += cost
...
plt.grid(True, linestyle="--", alpha=0.6)
if not os.environ.get("HIDE_PLOTS", False):
    plt.savefig("submission_4d.png")  # Save the plot
if not os.environ.get("HIDE_PLOTS", False):
    plt.show()
