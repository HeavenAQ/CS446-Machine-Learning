...
print(
    f"[DTypes] Numerical: {len(numerical_features)} | Categorical: {len(categorical_features)}"
)
...
def impute_data(
    ...
    if strategy == "median":
        ...
        filler = X_train[num_cols].median()
    elif strategy == "mode":
        # TODO a(ii) calculate mean
        cat_cols = X_train.select_dtypes(exclude=[np.number]).columns
        filler = X_train[cat_cols].mode().iloc[0]
    else:
        raise ValueError("strategy must be 'median' or 'mode'")

    # TODO a(ii) use fillna
    X_train.loc[:, features] = X_train.fillna(filler)
    X_val.loc[:, features] = X_val.fillna(filler)
    X_test.loc[:, features] = X_test.fillna(filler)

    return X_train, X_val, X_test


# Numerical imputation with median
X_train, X_val, X_test = impute_data(
    X_train, X_val, X_test, numerical_features, strategy="median"
)

# Categorical imputation with mode
if len(categorical_features) > 0:
    X_train, X_val, X_test = impute_data(
        X_train, X_val, X_test, categorical_features, strategy="mode"
    )
...
if len(categorical_features) > 0:
    X_train_cat = pd.get_dummies(
        X_train[categorical_features], drop_first=True, dtype=float
    )
    X_val_cat = pd.get_dummies(
        X_val[categorical_features], drop_first=True, dtype=float
    )
    X_test_cat = pd.get_dummies(
        X_test[categorical_features], drop_first=True, dtype=float
    )
...
def zscore_transform(X: pd.DataFrame, mean, std):
    """
...
def ridge_solve_predict(XtX, Xty, I, best_lambda, X_test_b):
    ...
    """
    w_ridge = np.linalg.pinv(XtX + best_lambda * I) @ Xty
...
if SHOW_PLOTS:
    ...
    plt.ylabel("MSE")
    plt.grid(True, ls="--")
    plt.title("Ridge: Train/Val MSE vs λ")
...
print(
    f"[Lasso-ISTA] Best λ={best_lambda_lasso:.2e} | Test MSE: {mse_lasso:.4f} | RMSE ($): {rmse_lasso:,.2f}"
)
print(
    f"[Sparsity] OLS={nonzeros_ols}/{total_features}, Ridge={nonzeros_ridge}/{total_features}, Lasso={nonzeros_lasso}/{total_features}"
)
...
print(
    f"[Ridge-LOG] Best λ={best_lambda_ridge_log} | MSE: {mse_ridge_log_back:.4f} | RMSE ($): {rmse_ridge_log_back:,.2f}"
)
print(
    f"[Lasso-LOG] Best λ={best_lambda_lasso_log:.2e} | MSE: {mse_lasso_log_back:.4f} | RMSE ($): {rmse_lasso_log_back:,.2f}"
)
