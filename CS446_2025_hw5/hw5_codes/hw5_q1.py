...
class DecisionTreeClassifier:
    ...
    def _grow_tree(self, X: NDArray, y: NDArray, depth=0):
        ...
        # --- PERF: STUDENT IMPLEMENTATION (1/7) ---

        # Define the stopping criteria for the recursion.
        # PERF: Check if the current depth at least the maximum depth.
        is_max_depth_reached = self.max_depth == depth
        # PERF: Check if there is only one unique class label at this node.
        is_one_class = len(np.unique(y)) == 1
        # PERF: Check if there are fewer samples at this node than the minimum required for a split.
        ...
        # PERF: Find the best split for the current data
        ...
        if best_feature is not None and best_threshold is not None:
            # PERF: Split the dataset into left and right subsets.
            # Hint: Call the self._split with appropriate arguments.
            left_idxs, right_idxs = self._split(X[:, best_feature], best_threshold)

            # PERF: Grow the left and right child nodes recursively.
    ...
    def _find_best_split(
        ...
        for feat_idx in range(n_features):
            ...
            for thr in thresholds:
                ...
                # PERF: Check if the gain is strictly better than the best found so far.
                is_better_gain = gain > best_gain
                if is_better_gain:
                    # PERF: Update current best gain, feature, and threshold.
                    best_gain = gain
    ...
    def _information_gain(self, y: NDArray, X_column: NDArray, threshold: float):
        ...
        """
        # --- PERF: STUDENT IMPLEMENTATION (3/7) ---

        # PERF: Calculate the Gini impurity of the parent node.
        # Hint: Call self._gini with appropriate arguments.
        parent_impurity = self._gini(y)

        # PERF: Split the data to get indices for left and right children.
        ...
        # PERF: Compute the Gini impurity for the left and right child nodes.
        # Hint: Call self._gini on the corresponding subsets.
        gini_left = self._gini(y[left_idxs])
        gini_right = self._gini(y[right_idxs])

        # PERF: Calculate the weighted average impurity of the children.
        ...
        # PERF: Compute the information gain.
    ...
    def _split(self, X_column: NDArray, split_thresh: float) -> Tuple[NDArray, NDArray]:
        ...
        """
        # --- PERF: STUDENT IMPLEMENTATION (4/7) ---

        # PERF: Find indices corresponding to the two nodes after the split.
    ...
    def _gini(self, y: NDArray):
        ...
        """
        # --- PERF: STUDENT IMPLEMENTATION (5/7) ---

        # PERF: Get the counts of each unique label in y.
        ...
        # PERF: Calculate the probabilities of each class.
        # Hint: Divide the counts by the total number of samples.
        probabilities = np.array(counts / y.size)

        # PERF: Compute the Gini impurity.
    ...
    def _most_common_label(self, y: NDArray):
        ...
        """
        # --- PERF: STUDENT IMPLEMENTATION (6/7) ---

        # PERF: Find the most common label in y. Return the smallest label in case of a tie.
        # Hint: np.bincount and np.argmax can be helpful.
        y_uniques, y_counts = np.unique_counts(y)
        most_common = y_uniques[y_counts.argmax()]
    ...
    def _traverse_tree(self, x: NDArray, node: Node | None):
        ...
        """
        # --- PERF: STUDENT IMPLEMENTATION (7/7) ---
        if node is None:
            return -1

        # If we have reached a leaf node, return its value.
        # PERF: Check if the current node is a leaf node.
        # Hint: Use a method of the Node class.
        is_leaf_node = False
        if node.is_leaf_node():
            is_leaf_node = True
        if is_leaf_node:
            # PERF: Return the value of the leaf node.
            # Hint: Access the node's value attribute.
            return node.value

        # PERF: Decide whether to go to the left or right child.
        # Hint: Check the sample's value for the feature the node splits on
        #       against the node's threshold (x_j <= threshold -> go_left = True).
        go_left = False
        if x[node.feature] <= node.threshold:
            go_left = True

        # PERF: Go to the left or right child accordingly.
